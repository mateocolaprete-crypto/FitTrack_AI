...
# Conexión a Google Sheets y Gemini
conn = st.connection("gsheets", type=GSheetsConnection)
# Intenta obtener la clave de secrets, si no, usa un string vacío para no romper todo el script de una
...
# 1. Configura la llave desde tus secrets
genai.configure(api_key=st.secrets["GEMINI_API_KEY"])

# 2. Configura el modelo con un "plan de respaldo" por si falla el nombre
try:
    # Intento estándar (el más limpio)
    model = genai.GenerativeModel('gemini-1.5-flash')
except Exception:
    # Plan B: Si la librería se pone caprichosa, usa el nombre de sistema
    ...
    if input_usuario:
        with st.status("Gemini analizando ingredientes...", expanded=True) as status:
            try:
                ...
                # Limpieza pro del JSON (extrae lo que esté entre llaves)
...
if "temp_data" in st.session_state:
    ...
    # Mostrar resultados en métricas estilizadas
    ...
    if st.button("💾 Confirmar y Registrar en mi Diario", type="primary"):
        try:
            ...
            del st.session_state.temp_data # Limpiar para la próxima carga
